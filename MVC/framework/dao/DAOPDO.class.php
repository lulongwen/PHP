  ...
  class DAOPDO implements iPDO{
    ...
    public function fetchOne($sql) {
      ...
      if( $pdo_statement === false ){
        ...
        echo '<mark>fetchOne SQL语句错误，错误是：</mark>'.$error[2] .'<br>';
        return false;
      }
    ...
    public function fetchAll($sql) {
      ...
      if($pdo_statement === false){
        $error = $this->_pdo->errorInfo();
        echo '<mark>fetchAll SQL语句错误，错误是：</mark>'.$error[2].'<br>';
        return false;
      }
    ...
    public function fetchColumn($sql) {
      ...
      if($pdo_statement === false){
        $error = $this->_pdo->errorInfo();
        echo '<mark>fetchColumn SQL语句错误，错误是：</mark>'.$error[2].'<br>';
        return false;
      }
      /**
       * 因为查询的sql语句如下：
    ...
    public function exec($sql) {
      ...
      if($result === false){
        $error =  $this->_pdo->errorInfo();
        echo '<mark>exec SQL语句错误，错误是：</mark>'.$error[2];
        return false;
      }
